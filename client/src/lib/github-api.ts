...
export async function fetchGitHubRepos(username: string): Promise<GitHubRepo[]> {
  try {
    ...
    return repos
      ...
      .sort((a, b) => {
        const aScore = a.stargazers_count * 2 + a.forks_count + (Date.now() - new Date(a.updated_at).getTime()) / (1000 * 60 * 60 * 24 * 365);
        const bScore = b.stargazers_count * 2 + b.forks_count + (Date.now() - new Date(b.updated_at).getTime()) / (1000 * 60 * 60 * 24 * 365);
